...
vector<Dino> Storage::getAllDinos()
{
    return this->dinos_;
}

optional<Dino> Storage::getDinoById(int Dino_id)
{
...
bool Storage::removeDino(int Dino_id)
{
    int index = -1;
...
int Storage::insertDino(const Dino &dvno)
{
...
bool Storage::updateDino(const Dino &dvno)
{
    int Dino_id = dvno.id;
    for(Dino & c : this->dinos_)
    {  
...
vector<Per> Storage::getAllPers()
{
    return this->pers_;
}

optional<Per> Storage::getPerById(int id)
{
    for(Per & p : this->pers_)
    {  
        if(p.id == id)
...
int Storage::insertPer(const Per &pr)
{
...
bool Storage::updatePer(const Per &pr)
{
    int Per_id = pr.id;
    for(Per & p : this->pers_)
    {  
