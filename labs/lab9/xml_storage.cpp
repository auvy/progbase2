...
vector<Dino> XmlStorage::getAllUserDino(int user_id)
{
    vector<Dino> dinos;
    Dino d;
    d.id = 0;
...
vector<Per> XmlStorage::getAllDinoPer(int dino_id)
{
    vector<Per> pers;
    Per p;
    p.id = 0;
...
bool XmlStorage::insertDinoPer(int dino_id, int per_id)
{
    return false;
}


bool XmlStorage::removeDinoPer(int dino_id, int per_id)
{
    return false;
}
